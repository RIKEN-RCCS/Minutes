...
| モデル | 結果 | 備考 |
|---|---|---|
| **Nvidia Nemotron-3-Super-120B** | △ 実用的だが品質課題あり | 単一パス（マルチステージ未実装時点）。Lost in the Middle で会議後半が欠落しやすい。instruction following が不安定。`--think` は推論が終了しなくなるため非推奨 |
...
| **Qwen3-Swallow-30B-A3B-RL-v0.2**（MoE） | ◎ 現在の推奨 | MoE により dense 比約2倍速（約20分/会議）。品質は dense と同等。`--think --no-chat-template-kwargs --max-tokens 16384` |
