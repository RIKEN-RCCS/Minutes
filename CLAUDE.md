...
MODEL_REMOTE = "openai/whisper-large-v3-turbo"  # リモート推論用モデル
...
- **ファイル検索とリトライ**: `files_list(channel=channel_id)` でチャンネル内を検索してファイル名で特定。アップロード直後は API に反映されない場合があるため、ファイルが見つからない間は最大 5 回（5 秒間隔）リトライする。
